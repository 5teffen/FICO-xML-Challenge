...
def anchor_generator(pre_proc_file, all_data_file, anchs_lst):
	...
	names = ["External Risk Estimate","Months Since Oldest Trade Open","Months Since Last Trade Open"
		...
		,"Trades Open Last 12M","% Installment Trades", "Months Since Most Recent Inq","Inq Last 6 Months"
		,"Inq Last 6 Months exl. 7 days", "Revolving Burden","Installment Burden","Revolving Trades w/ Balance"
		,"Installment Trades w/ Balance","Bank Trades w/ High Utilization Ratio","% Trades with Dalance"]
...
all_results, sample_lst = changes_generator("pre_data1.csv",combinations[1])

# names, good_squares, bad_squares, good_samp, bad_samp = anchor_generator("pre_data1.csv","final_data_file.csv",[2,3,21])

testing = prep_for_D3_global("pre_data1.csv","final_data_file.csv", [2012,3147,3999 ], bins_centred, X_pos_array,trans_dict)
